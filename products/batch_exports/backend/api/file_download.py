...
def count_rows_for_hogql_batch_export(team: Team, hogql_query: str, timeout: int = 30) -> int:
    ...
    # `execute_hogql_query` resolves the query with team-default modifiers, but the worker
    # resolves it with plain default modifiers (`create_hogql_context_for_batch_export`), so
    # modifier-sensitive queries can count slightly differently than they export. Acceptable
    # for an estimate.
...
class FileDownloadBatchExportOnDemandViewSet(
    ...
    @action(
        methods=["POST"],
        detail=False,
        required_scopes=["batch_export:read"],
