...
from products.tasks.backend.webhooks import _transition_signal_reports_for_task
...
def _link_task(team, report, *, pr_url, relationship=TASK_RUN_TYPE_IMPLEMENTATION):
    ...
    TaskRun.objects.create(team=team, task=task, output={"pr_url": pr_url})
    return task
...
def test_closing_a_superseded_pr_does_not_archive_the_report(team, report):
    ...
    old_task = _link_task(team, report, pr_url=_OLD_PR)
    _link_task(team, report, pr_url=_NEW_PR)

    _transition_signal_reports_for_task(
        old_task.id, _OLD_PR, SignalReport.Status.SUPPRESSED, "archived", skip_superseded=True
    )
...
def test_closing_the_newest_pr_still_archives_the_report(team, report):
    task = _link_task(team, report, pr_url=_OLD_PR)

    _transition_signal_reports_for_task(
        task.id, _OLD_PR, SignalReport.Status.SUPPRESSED, "archived", skip_superseded=True
    )
