...
from products.tasks.backend.facade.contracts import RepositoryPublishingError, SignedRepositoryCommit
...
def _staged_file_contents(sandbox: SandboxBase, repository_path: str, path: str) -> str:
    ...
    if b"\x00" in decoded:
        raise RepositoryPublishingError(f"Repository file '{path}' is binary.")
    return contents
